...
CC -O3 -DUSE_ALGOR=4 -DSELECT_BLAS=1 \
	-DUSE_PAT_API=2 -DWRITE_ARRAYS=0 \
	-o ./matmul_mkl.exe -I. ../../src/matmul_main.cpp -qmkl=sequential

pat_build -o ./matmul_mkl_pat.exe ./matmul_mkl.exe
...
echo "--> PAT_RT_PERFCTR=${PAT_RT_PERFCTR} <---" >& ref_program.out

srun ./matmul_mkl_pat.exe >> ref_program.out
pat_report ./matmul_mkl_pat.exe+* >& ref_pat_inst.out

rm -rf ./matmul_mkl_pat.exe+*
...
srun ./matmul_mkl_pat.exe >> ref_program.out
pat_report ./matmul_mkl_pat.exe+* >& ref_pat_cache.out
